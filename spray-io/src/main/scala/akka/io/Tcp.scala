...
object Tcp extends ExtensionKey[TcpExt] {
  ...
  trait Write extends Command {
    ...
    def copy(data: ByteString = data, ack: Any = ack): Write
  }
  case class DefaultWrite(data: ByteString, ack: Any) extends Write {
    require(ack != null, "ack must be non-null. Use NoAck if you don't want acks.")
    def copy(data: ByteString = data, ack: Any = ack): Write = DefaultWrite(data, ack)
  }
  object Write {
    val Empty: Write = DefaultWrite(ByteString.empty, NoAck)
    def apply(data: ByteString): Write =
      if (data.isEmpty) Empty else DefaultWrite(data, NoAck)
    def apply(data: ByteString, ack: Any): DefaultWrite = DefaultWrite(data, ack)
  }
...
class TcpExt(system: ExtendedActorSystem) extends akka.io.IO.Extension {
